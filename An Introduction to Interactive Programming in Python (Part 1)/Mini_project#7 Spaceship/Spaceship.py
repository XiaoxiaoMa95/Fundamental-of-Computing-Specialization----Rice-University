...
# http://www.codeskulptor.org/#user47_RhYClUVfEFpkg07.py
...
class Ship:
    def __init__(self, pos, vel, angle, image, info):
        ...
        self.thrust = False  # whether thrust, up arrow
        self.angle = angle
        self.angle_vel = 0  # left/right arrow
        self.image = image
    ...
    def update(self):
        ...
        friction_cff = 0.05
        self.vel[0] *= (1 - friction_cff)
        self.vel[1] *= (1 - friction_cff)
    ...
    def thruster(self, isthruster):
        # turn the thrusters on/off
        self.thrust = isthruster
        if self.thrust:
...
def draw(canvas):
    ...
    canvas.draw_text('Score = ' + str(score), [500, 50], 30, 'White')
...
def rock_spawner():
    ...
    vel_x = random.choice([0.5, -1]) * random.random() * 2
    vel_y = random.choice([0.5, -1]) * random.random() * 2
    a_rock.vel = [vel_x, vel_y]

    a_rock.angle_vel = random.choice([0.5, -1]) * 0.05
